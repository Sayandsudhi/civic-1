...
export default function OfficerLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const data = await authService.loginOfficer(username, password);
      if (data.user.role !== "DEPARTMENT_OFFICER") {
        const dest = data.user.role === "ADMIN" ? "/admin" : "/login";
        setError(`Access denied: This portal is strictly for Department Officers. Please use ${dest}.`);
        showToast("Unauthorized portal access.", "error");
        return;
      }
      login(data.access_token, data.user);
      showToast(`Welcome Officer ${data.user.full_name} (${data.user.department_name})`, "success");
      const from = location.state?.from?.pathname || "/department/dashboard";
      navigate(from, { replace: true });
    } catch (err) {
      const msg = err.response?.data?.detail || err.message || "Invalid officer credentials.";
      setError(msg);
      showToast(msg, "error");
    } finally {
      setLoading(false);
    ...
    <div className="min-h-[85vh] flex items-center justify-center p-4">
      <div className="w-full max-w-md rounded-2xl bg-slate-900/90 border border-slate-800 p-8 shadow-xl relative">
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold tracking-tight text-white">Department Operations</h2>
          <p className="text-xs text-slate-400 mt-1">Authorized municipal field squads and department officers.</p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="officer_kseb or kseb.officer@civicpulse.gov"
